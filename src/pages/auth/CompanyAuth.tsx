...
import { ArrowLeft, Briefcase, Building2, Eye, EyeOff, Lock, Mail } from "lucide-react";
import { useEffect, useState } from "react";
import { Link, useNavigate, useSearchParams } from "react-router-dom";

const REMEMBER_EMAIL_KEY = "joblink_company_email";
...
export default function CompanyAuth() {
  ...
  const checkUserRoleAndRedirect = async (userId: string) => {
    ...
    } else if (roleData?.role === "jobseeker") {
      // Jobseeker trying to login on company page
      await supabase.auth.signOut();
      toast({
        title: "登录失败",
        description: "求职者请在求职者登录页面登录",
        variant: "destructive",
      });
      ...
      if (isLogin) {
        ...
        if (data.user) {
          ...
          if (!roleData?.role) {
            ...
            if (insertRoleError) {
              console.error("Failed to auto create company role:", insertRoleError);
              throw new Error("初始化用户角色失败，请联系管理员或稍后重试。");
            }
          }

          if (roleData?.role !== "company" && roleData?.role !== "admin") {
            await supabase.auth.signOut();
            throw new Error("求职者请在求职者登录页面登录");
          }
        }
        ...
        if (data.user) {
          ...
          if (profileError) {
            ...
            throw new Error(`创建企业档案失败: ${profileError.message}`);
          }
    ...
    <div className="min-h-screen hero-bg flex flex-col">
      ...
      <main className="flex-1 container flex items-center justify-center py-12">
        <Card className="w-full max-w-md p-8 animate-scale-in">
          ...
          {/* User Type Badge */}
          <div className="flex items-center justify-center gap-2 mb-6 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
            <Building2 className="w-5 h-5 text-blue-500" />
            <span className="font-medium text-blue-500">企业登录</span>
          </div>
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">
              {isLogin ? "登录后管理您的招聘需求" : "入驻 JobLink，发现优秀人才"}
            </p>
          </div>
          ...
          <div className="mt-4 text-center">
            <Link to="/auth/jobseeker" className="text-sm text-muted-foreground hover:text-primary">
              求职者？点击这里登录
            </Link>
          </div>
        </Card>
